...
- **v0.2 — FEATURE-COMPLETE; deploy pending.** All four playground blocks built (**hero** with per-letter cursor repel, **tech-stack** with an icon orbit, **microfrontend-meta** with live Counter + diagram + skeleton loader + simulate-offline, **presence-room** with multiplayer cursors via PartyKit). Header + footer frame in place. Resilience added: MF runtime fallback plugin so an unreachable remote no longer blanks the host; top-level `ErrorBoundary` + `unhandledrejection` handler; `loadRemote` instead of `import('lab/X')` so blocks lazy-load _after_ React mounts. **Scroll-reveal** on every cell (`useInView` hook + Tailwind transition, IO-based; no Framer Motion) and the MicrofrontendBlock's federated chunk literally defers until the cell scrolls into view (`RemoteMount` gates its `load()` on `inView`) — the on-page copy "fetched the moment it scrolled into view" is now true. **Dark mode toggle** added (light / dark / system) via shadcn `DropdownMenu` in the header; inline init script in `index.html` mirrors `useTheme` so the right class lands on `<html>` before any paint (kills FOUC and theme-flash). Lab inherits theme via CSS-variable cascade — no remote changes needed. Deploy plumbing prepared: federation `entry` and `VITE_PARTY_HOST` env-driven via `loadEnv` + a `dev:tunnel` mode that reads `.env.tunnel.local`; lab `preview` has `cors: true` + `allowedHosts: true` for cross-origin script fetches. **React Compiler** integrated on the host via a custom Vite plugin (the documented `@rolldown/plugin-babel` + `reactCompilerPreset` path silently fails in this project — see `module-federation-setup.md` §12). Remaining: actual deploy (host + remote to static hosting, party to a stateful host).
...
- Tailwind. (Framer Motion was in the original spec but never used — scroll-reveal landed on plain CSS + `IntersectionObserver`, see `module-federation-setup.md` §14.)
- Light / dark / system theme toggle in the host header (shadcn `DropdownMenu`, persisted to `localStorage`). Defaults to system preference. Near-black dark bg with a slight cool tint, off-white headings, muted-gray body, one sparingly-used accent color (green/cyan family reads "technical"). Light mode is a mirror via the existing CSS-variable theme — remotes inherit via cascade.
...
**Behavior:** cells fade-and-rise on scroll (delivered — `useInView` + Tailwind transition, no Framer Motion); fully responsive (cells stack full-width on mobile); respect `prefers-reduced-motion`; don't break the existing MF setup.
...
- **i18n**: each remote owns its own translations (its own i18n library, its own translation files). Only the locale STRING crosses the boundary, propagated via the `<html lang>` attribute (host owns it). Conventions shared, implementations independent. **Host shipped:** `i18next` + `react-i18next` + `i18next-browser-languagedetector`, English + Indonesian (Indonesian strings are draft — awaiting native-speaker review). `localStorage` key `locale`, falls back to `navigator.language`. `LocaleToggle` (shadcn `ToggleGroup`, EN/ID pill) lives in the header next to `ThemeToggle`. `<html lang>` is set on init and on `languageChanged` event. **Lab not yet translated** — next task; will use `svelte-i18n` and read `opts.locale` from `mountCounter`/`mountPresence`. The forward-compat `opts` hook is the wire.
- **Dark mode**: pure CSS variables. Host toggles a class on `<html>`; every remote inherits it through the CSS cascade automatically, because remotes render into the host's DOM. Zero coordination code, no events, no shared store. The v0.2 toggle (light / dark / system) is a shadcn `DropdownMenu` in the header backed by `src/lib/useTheme.ts`; inline init script in `index.html` reads the same `localStorage` key + `prefers-color-scheme` to set the class before paint (no FOUC, no theme-flash). The standalone lab page stays dark-only — toggle lives on the host because that's the integration surface that owns visitor preference.
