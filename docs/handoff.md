...
2. **Harden presence** — ghost-replay / synthetic cursors for the empty-room problem. A portfolio usually has one visitor, so the deployed presence demo shows a solo recruiter an otherwise empty room — arguably higher priority than any new block, since it's the demo most likely to fall flat for the exact audience it's built for. (Partially softened: the count pill now shows the visitor their own assigned identity from the server's `welcome`, and stale-ghost hygiene — reconnect reset + a 5-minute peer sweep — shipped; see [features.md](./features.md). The synthetic-cursor half is still open.)
