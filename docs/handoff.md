...
- **v0.3 — content. (in progress)** Next.js blog scaffolded; CV page built; MDX posts pipeline built (`@next/mdx` + `remark-gfm`, `posts/[slug]`, `prose` styling, a template post seeding it); **deployed at `naufal-blog.pages.dev`** (SEO basics — `sitemap.xml`/`robots.txt`/OG — and the Carlito print fix included). **Still pending:** the first real technical post (the MF build itself — the [gotchas.md](./gotchas.md) list is the raw material) and the experience "stories."
