...
Naufal Yusran, software engineer in Jakarta. Frontend-leaning fullstack, ~3 years professional experience. Working at DBS Bank Indonesia (Feb 2026 – present) as a React.js Developer on a webview-based microfrontend app using Module Federation. Prior: Ajaib (Next.js landing pages + onboarding webview), Bank Danamon (DBankPro 2.0 back office, React container + Angular feature modules with Single-SPA → NX migration), Infosys Solusi Terpadu (a loan origination system for Sarana Multi Infrastruktur, a BUMN — Next.js + Material UI; also the vendor behind the Danamon contract), Doubler Studio (BCA projects — vanilla JS/jQuery/Nunjucks/Gulp on contract, then React/Next as a parallel freelance), GeekGarden (Flutter), Ehealth (Polymer), Traveloka intern.
...
- **v0.3 — content (in progress).** The standalone Next.js blog: branded home, pixel-faithful CV, the MDX posts pipeline, SEO basics, EN/ID i18n, theming, and a reading panel — all deployed, with two real posts live. **The gap that still defines this phase: the experience "stories"** (a `/work/<slug>` per job, deep-linked from the career graph). [gotchas.md](./gotchas.md) remains the raw material for the next technical posts.
...
1. **First blog post — DONE.** `styling-across-module-federation` shipped in both locales, drawn from [gotchas.md](./gotchas.md) (which stays the running learnings log — the role the old `LEARNINGS.md` task was meant to fill). The pipeline is proven; the TS6/baseUrl trap and the eager-preload / `loadRemote` pattern remain publishable follow-ups.
