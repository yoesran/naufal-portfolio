...
- **Dark mode**: pure CSS variables. Host toggles a class on `<html>`; every remote inherits it through the cascade — zero coordination code, no events, no shared store. v0.2 toggle (light / dark / system) is a shadcn `DropdownMenu` backed by `useTheme.ts`; inline init script kills FOUC + theme-flash. Standalone lab stays dark-only. Full detail in [features.md](./features.md).
