...
**Shared store, not per-component state.** All five controls live in the theme-lab block, but they're backed by a module store, [`theme.ts`](../naufal-host/src/lib/theme.ts), instead of local state — read via `useSyncExternalStore`, so the block, the OS-theme listener, and the generated pre-paint script all stay in sync off one source of truth. A single `apply()` writes the whole config onto `<html>` (`.dark` class, `data-accent`, `data-surface`, inline `--radius`, inline `--font-app`). It also listens for OS-theme flips while in `system` mode. (This replaced the earlier per-instance `useTheme`/`useAccent` hooks.)
