...
- The PartyKit server (`naufal-party/src/server.ts`) is a tiny relay: on connect it assigns a colour + friendly name (sent back to that connection as a `welcome` message); on a cursor message it broadcasts `{ id, color, name, context, x, y }` to everyone else; on disconnect it broadcasts `leave`.
- The Svelte component connects with `partysocket`, sends viewport-normalized (0–1) cursor coords on `mousemove` (rAF-throttled), and renders peers' cursors with a name pill + origin tag. The visitor's **own** identity (assigned by the server, returned in `welcome`) is visible too: a name tag rides their native cursor (no drawn arrow — the real pointer is the arrow — and no position transition, so it tracks with zero lag; never set on touch devices, which have no cursor) tagged "· you", and the count pill repeats it ("you're Otter" + colour dot). A solo visitor therefore sees the presence system working — exactly what peers would see — before anyone else joins.
