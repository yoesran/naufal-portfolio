...
- The base-mode UI lives only in the theme-lab block (see below) — `mode` / `setMode` off `useThemeConfig()`, a light/dark/system control alongside the other four axes. There is no longer a standalone header toggle (the earlier `ThemeToggle`/`PaletteToggle` header components were removed; the block is the single place to change theme).
...
## Theme customizer (the `theme-lab` block)

The host is fully visitor-themeable across five axes — **base mode** (light/dark/system), **accent**, **background/surface**, **corner radius**, and **font** — and every axis is just a value on the host's `<html>`, so it **cascades into the federated Svelte remote live**. That's what makes [`ThemeLabBlock`](../naufal-host/src/components/blocks/ThemeLabBlock.tsx) a working demo of the CSS-variables-cross-the-MF-boundary claim rather than just an assertion: re-skin the page and watch the embedded Counter re-skin with it, no remount.

**Shared store, not per-component state.** Controls live in two places (the header toggles and the block), so they're backed by one module store, [`theme.ts`](../naufal-host/src/lib/theme.ts), read via `useSyncExternalStore` — every control stays in sync, and a single `apply()` writes the whole config onto `<html>` (`.dark` class, `data-accent`, `data-surface`, inline `--radius`, inline `--font-app`). It also listens for OS-theme flips while in `system` mode. (This replaced the earlier per-instance `useTheme`/`useAccent` hooks.)
...
**No-FOUC.** The generated inline `<head>` script (see the dark-mode section — built from [`theme-tokens.ts`](../naufal-host/src/lib/theme-tokens.ts) by the `theme-prepaint` Vite plugin) restores all five before first paint, including a small background lookup so a custom surface doesn't flash the default bg before CSS loads. **Status colors stay literal** (connected/connecting/offline greens/ambers/reds) — state, not theme. **Remote side:** [Counter.svelte](../naufal-lab/src/lib/Counter.svelte)'s embedded branch uses `brand` utilities + `color-mix(in oklch, var(--brand) …)`; the lab's [app.css](../naufal-lab/src/app.css) maps `--color-brand: var(--brand, <emerald>)` so it resolves to the host value when embedded and a fallback standalone. The theme-lab block is the single entry point for all five axes — the header carries only the `LocaleToggle` (the quick `ThemeToggle`/`PaletteToggle` header shortcuts were removed).
...
- [`src/components/LocaleToggle.tsx`](../naufal-host/src/components/LocaleToggle.tsx) — shadcn `ToggleGroup` (binary "EN / ID" pill, segmented-control look with `spacing={0}`). Lives in the header (the only control there now, beside the mobile nav menu). Maps over `LOCALES` so adding a third locale to `i18n.ts` automatically adds a third toggle item.
