...
Two more Pages projects ride the same account but aren't part of the MF system: **`naufal-reports`** (the published test reports — see _Quality dashboard_ below) and **`sukamotret`** (the client site; it deploys the same `next build` → `wrangler pages deploy out` way, documented in its own [README](../sukamotret/README.md)).
