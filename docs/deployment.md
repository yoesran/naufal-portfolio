...
The host's `// quality` block surfaces the host's own two test suites — real **Vitest** (unit + RTL component) and **Playwright** (e2e, with video) runs, each linking to its full HTML report — plus a card per project for the **rest of the workspace** (each sibling's own Vitest: `naufal-lab`, `naufal-blog`, `naufal-party`), each linking to that project's own published report too. Published **manually** (no CI, no secret — `wrangler` is already OAuth-authed locally) to a dedicated Pages project:
