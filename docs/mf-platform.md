...
## Initial-load performance: runtime-registered remote + modulepreload + code-splitting

Measured (2026-07, prod builds): even with the `loadRemote` opt-out, deployed FCP sat at **~4.4s**. Two causes, two fixes:

- **The lab was still a build-config remote**, so the generated bootstrap fetched its cross-origin `remoteEntry.js` (plus the lab's virtual-module follow-ons) _before_ `main.tsx` executed — ~1s of deployed FCP, and a **3s connection-timeout stall locally** whenever the lab wasn't running. Fixed by removing `remotes` from the host's federation config entirely: the lab is **registered at runtime** ([`lab-remote.ts`](../naufal-host/src/lib/lab-remote.ts) `ensureLabRemote()`, called by `LiveRemoteBlock` and `PresenceOverlay` right before their `loadRemote`). Nothing lab-related is fetched until a federated interaction; a down lab affects only those blocks' fallbacks, never first paint. Type note: `lab/*` types come from the local `@mf-types/` snapshot (tsconfig `paths`); to refresh them after the lab's exposed API changes, temporarily restore `remotes` + `dts.consumeTypes` in the host config and run dev with the lab up.
- **The MF bootstrap itself is a serial chain** of tiny virtual modules (`hostInit` → host remoteEntry shim → shared-import map → `loadShare(react)` → runtime → `loadShare(react-dom)` → the real entry) — each discovered only after the previous executes, ~12 network round trips that Vite's own modulepreload emission can't see through. The **`preload-critical` plugin** (host and lab `vite.config.ts` — duplicated by hand, the repos share no tooling) walks the bundle at build time and injects `<link rel="modulepreload">` for the whole boot graph (MF virtual modules + the app entry + its static imports — not the lazy block chunks), plus a `preload` for the latin Inter woff2. The chain then fetches as one parallel burst. Result: host FCP 3.9s → **~1.0s** local; the lab standalone page got the same treatment (its whole boot graph completes in ~55ms local).

Two earlier host-side optimisations still apply beneath those:
...
- **Preconnect to the remote origin** ([`vite.config.ts`](../naufal-host/vite.config.ts), `preconnect-remote` plugin). A `<link rel="preconnect" crossorigin>` to `VITE_LAB_URL` is injected into `<head>` so the cross-origin `remoteEntry.js` fetch (now fired on the first federated interaction, not at boot) skips the DNS+TLS handshake. Env-aware (dev → `127.0.0.1:5174`, prod → the deployed origin); `crossorigin` matches the module script's anonymous CORS fetch.
